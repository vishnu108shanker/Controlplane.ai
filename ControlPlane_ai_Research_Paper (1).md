...
**A design-rationale paper documenting the ideation journey, architecture, and prototype plan**
Prepared for the Accenture Innovation Challenge 2026
...
|---|---|---|---|
| **A — Live TV / streaming intervention** | Inspect and redact AI output mid-stream (e.g. redact an SSN as it streams) | Demoted to supporting capability | PII redaction and runtime content filtering are already mature commodity capabilities. |
| **B — Traffic-light intervention** | RED/YELLOW/GREEN mapping to block/warn/allow | Demoted to UX concept | Risk-graded allow/block/modify/escalate is standard guardrail behavior industry-wide. |
| **C — Micro-escalation** | Live Slack/Teams approval request with countdown, instead of a ticket queue | Kept as possible supporting feature | Improves UX of human-in-the-loop but is not itself a defensible innovation. |
| **D — Liability Black Box** | Every intervention produces a forensic record (prompt, response, policy, evidence, decision, timestamps, approvals) for legal/insurance defensibility | Kept as necessary infrastructure | Audit trails and decision logs are already baseline in observability/evaluation tooling — required, not differentiating. |
| **E — Behavioral Contracts** | Explicit business rules such as "never quote a price without a database lookup" or "never issue a refund above threshold without approval" | Kept as conceptual ancestor of the final concept | First serious move away from generic safety toward business-specific rules — but a static contract is still policy-as-code, and invites the question "who wrote the contract?" This question became the doorway to the final concept. |
| **F — Consensus Gate** | High-risk agent actions require a second independent model/checker to co-sign before executing | Kept as a possible future extension | Visually compelling and relevant to agentic AI, but does not address the deeper problem of *evolving* organizational policy. |
| **G — Cost as an attack surface** | Treat recursive tool loops, retries, and wasteful model selection as a "denial-of-wallet" security problem | Kept as an evidence signal, not core | Reframes cost usefully but is not, alone, the central innovation. |
| **H — Message recall** | Deliver a response quickly, then let an asynchronous "deep lane" recall or edit it later | **Rejected outright** | A message may already have been read, and more fundamentally: **you cannot recall a side effect.** If an agent has already transferred money, deleted data, sent an email, or deployed code, editing the chat message afterward changes nothing. This surfaced a logical contradiction in any recall-based architecture. |
...
→ pattern discovery
→ "Premium customer + airline fault + refund ≤ ₹15k is consistently
   approved and produces successful outcomes."
→ proposed policy
```
...
| Current-policy evaluation | Deterministic | This is exactly what a policy engine should do |
| Fuzzy risk scoring | ML / anomaly model | Requires pattern recognition across correlated features nobody would hand-code |
| Evidence grounding | Retrieval + bounded LLM comparison | The LLM's role is narrow: does claim X match retrieved fact Y — not "is this okay" |
| Pattern discovery | ML / statistics / interpretable rule mining | Discovers conditional structure nobody explicitly programmed |
| Policy explanation / diff | LLM (generative) | Legitimate use — advisory prose a human reviews before anything changes |
| Approved-policy compilation | Deterministic codegen + regression tests | The semantic decision was already made by a human; turning it into executable syntax is mechanical and testable |
| Outcome attribution | Data joins + statistics/ML | Ground truth should come from business data, not from asking a model if a decision "seems fine" |
...
4. **Policy proposal drafting** — turning discovered statistical structure into human-readable rationale, a policy diff, and suggested conditions, always subject to human review.
...
```text
1. Discover → 2. Measure → 3. Validate → 4. Explain → 5. Human review
→ 6. Regression test → 7. Version → 8. Deploy → 9. Monitor
```
...
DISCOVERED PATTERN
Premium customer + verified documentation + airline-caused
cancellation + claim ≤ ₹75,000
...
```text
                ...
                └──────────────┬───────────────┘
                      ...
                      REJECT  │   APPROVE
                               ▼
                       REGRESSION TEST → POLICY VERSION → RUNTIME
```
...
| Objection | Response |
|---|---|
| "Isn't this just an if-statement?" | The deterministic engine is an if-statement; the innovation is discovering *which* if-statement should exist, from evidence nobody hand-coded. |
| "Why use AI at all?" | AI is confined to pattern discovery, anomaly detection, evidence comparison, and proposal drafting — see Section 6.3. Remove the AI and the discovery loop stops working; the enforcement layer keeps working (by design). |
| "Why trust the LLM?" | The LLM never has enforcement authority. Its output is advisory prose reviewed by a human before any policy changes; worst case is a badly worded draft that gets rejected. |
| "What if the humans are wrong?" | The system learns from human decisions *combined with outcomes*, not from human decisions as ground truth — see Section 6.6. |
| "Why isn't this just policy-as-code?" | Policy-as-code starts from a human-written rule and compiles it. ControlPlane discovers the candidate rule from behavioral and outcome evidence first, and only then compiles the human-approved result into policy-as-code. |
| "Why not just use an existing guardrail product?" | Existing guardrails enforce policies someone already wrote. None of the reviewed competitors mine override/outcome history into a versioned, human-approved, executable policy proposal — see Section 4.4–4.5. |
| "Why not let the AI automatically change the policy?" | This was deliberately rejected — see the Human Authority gate in Section 6, and Lesson 5 below. Unilateral AI policy authority reintroduces exactly the "what if it learns bad behavior" risk the project is trying to avoid. |
...
**Settled:** vision, product thesis, differentiation claim, architecture (runtime/learning split), domain selection.
**Not yet designed:** the concrete prototype.
